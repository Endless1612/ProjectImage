...
public class RobertsCrossEdgeDetector implements EdgeDetector {
    ...
    private static final int[][] ROBERTS_CROSS_X = {
            {1, 0},
            {0, -1}
    };

    private static final int[][] ROBERTS_CROSS_Y = {
            {0, 1},
            {-1, 0}
    };
    ...
    public File detectEdges(File inputFile) throws IOException {
        ...
        double[][] gradientX = applyConvolution(grayImage, ROBERTS_CROSS_X);
        double[][] gradientY = applyConvolution(grayImage, ROBERTS_CROSS_Y);
        ...
        File outputFile = new File("roberts_cross_output.png");
    ...
    // Apply convolution with Roberts Cross kernel
    private double[][] applyConvolution(BufferedImage image, int[][] kernel) {
        ...
        int kernelSize = 2;  // Roberts Cross uses a 2x2 kernel
